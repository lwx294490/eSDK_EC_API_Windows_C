...
#define TC_SDK_VERSION_STRING		"TUP Conf V001R001C50SPC200B011"
...
enum
{
	CONF_MODE_PRODUCT,		//产品模式
	CONF_MODE_PAAS			//SAAS模式
};
...
typedef enum //会议控制选项
{
	...
	CONF_OPTION_VIDEO_DOUBLE_STREAM = 0x0010,		//视频支持双流，废弃：使用COMPT_OPTION_VIDEO_DOUBLE_STREAM
	CONF_OPTION_VIDEO_UDP			= 0x0020,		//视频采用UDP，废弃：使用COMPT_OPTION_VIDEO_UDP
	...
	CONF_OPTION_NEW_APPSHARE		= 0x1000,		//采用新版本屏幕共享
	CONF_OPTION_UC_AUDIO			= 0x2000,       //UC客户端，要带这个标记，音频模块对UC有特殊处理，废弃：使用COMPT_OPTION_AUDIO_UC
	CONF_OPTION_AUDIO_TCP			= 0x4000,		//音频采用TCP，废弃：使用COMPT_OPTION_AUDIO_TCP
...
typedef enum{
	COMPT_OPTION_VIDEO_DOUBLE_STREAM			= 101,		//视频支持双流,void* option int
	COMPT_OPTION_VIDEO_UDP						= 102,		//视频采用UDP, void* option int
	COMPT_OPTION_VIDEO_NOT_REPORT_DEVICE		= 103,		//是否要汇报设备信息，void* option int
	COMPT_OPTION_VIDEO_P2P						= 104,		//是否启用P2P传输，该模式下2方会议视频数据采用P2P传输 void* option int
	COMPT_OPTION_VIDEO_MAX_COUNT				= 105,		//最大打开视频数  void* option int
	COMPT_OPTION_VIDEO_MAX_ENCODE_RESOLUTION	= 106,		//最大编码分辨率  void* option TC_VIDEO_PARAM (里面的xResolution,yResolution)
	COMPT_OPTION_VIDEO_MIN_ENCODE_RESOLUTION	= 107,		//低流的分辨率    void* option TC_VIDEO_PARAM (里面的xResolution,yResolution)
	COMPT_OPTION_VIDEO_MAX_VIEW_RESOLUTION		= 108,		//观看的最大分辨率void* option TC_VIDEO_PARAM (里面的xResolution,yResolution)
	COMPT_OPTION_VIDEO_MAX_VIEW_COUNT			= 109,		//观看的高流个数，缺省为1 void* option int

	COMPT_OPTION_VIDEO_HARDWARE_ENCODE			= 110,		//缺省使用硬编，加上此选项使用软编	
	COMPT_OPTION_VIDEO_HARDWARE_DECODE			= 111,		//缺省使用软解，加上此选项使用硬解	


	COMPT_OPTION_AUDIO_UC						= 201,		//UC客户端
	COMPT_OPTION_AUDIO_TCP						= 202,		//音频采用TCP

	COMPT_OPTION_AS_P2P							= 301,		//是否启用P2P传输，该模式下2方会议屏幕共享数据采用P2P传输

	COMPT_OPTION_DS_WPS_DEFAULT					= 401,		//是否默认用WPS打开
	COMPT_OPTION_DS_BMP_NOZOOM					= 402,		//打开图片不缩放
}COMPT_OPTION;
...
enum{
	...
	TC_IMCC_RESULT_SERVER_CONN_FAILED		= 108,		//连接上级服务器失败
	TC_IMCC_RESULT_SYS_SHUTINGDOWN			= 109,		//连接断开
	TC_IMCC_RESULT_DOMAIN_MERGING			= 110,		//会议状态错误
	...
	TC_IMCC_RESULT_POOL_NOT_AVAILABLE		= 124,		//Audio打开MIC数量已超过设置的最大打开MIC数
	TC_IMCC_RESULT_AUTH_FAILED				= 126,		//鉴权失败
	TC_IMCC_RESULT_UNKNOWN					= 127,		//服务器返回未知错误
	TC_IMCC_RESULT_AUTH_REDIRECT			= 128,		
	TC_IMCC_RESULT_QUERY_PARENT_OR_TOP_AGENT_EXPIRED = 129, //集群服务异常
	TC_IMCC_RESULT_DOMAIN_LOCKED			= 130,		//会议锁定
	TC_IMCC_RESULT_AUTH_TIMEOUT				= 131,		//鉴权超时    
	TC_IMCC_RESULT_AUTH_KEY_NOT_AVAILABLE	= 132,		//会议密码错误
	TC_IMCC_RESULT_TOO_MANY_CONFERENCE		= 133,		//会议数超过限制     
	TC_IMCC_RESULT_TOO_MANY_CONFERENCE_USER = 134,		//会议成员超过限制
	TC_IMCC_RESULT_NOT_DATA_USER_LICENSE    = 135,		//没有足够的数据License，不允许入会
	...
	TC_IMCC_REASON_USER_INITIATED		= TC_IMCC_REASON_BASE,	//会议正常离会
	...
	TC_CONF_ERROR_PAAS_ID_INVALID		= 420,		//Paas模式下，错误的会议ID，会议前12位不能有值

    // 聊天错误码基数
	TC_CHAT_ERROR_BASE	                = 1000,
	TC_CHAT_ERROR_INVALIDSESSION        = 1001,		//通道未加入
	TC_CHAT_ERROR_INVALIDUSERID         = 1002,		//错误的用户ID
	TC_CHAT_ERROR_INVALIDGROUPID        = 1003,		//错误的组ID
	TC_CHAT_ERROR_TOO_MANY_CHAR         = 1004,		//字符串超过长度限制
	...
	TC_DS_ERROR_DE_NOTEXIST				= 1104,		//元素不存在
    ...
    TC_DS_ERROR_DOC_NO_SINK             = 1112,		//回调接口未设置
	...
	TC_DS_LOADER_PPT_THREAD_ERROR		= 1131,		//PPT加载线程错误
	TC_DS_LOADER_PPT_LOAD_ERROR			= 1132,		//PPT加载错误
	TC_DS_LOADER_PAGE_OVER_MAX			= 1133,		//超出最大页数(200页)
	...
	TC_DS_LOADER_WPS_NOT_INSTALLED		= 1138,		//WPS没有安装

	TC_ANNOT_ERROR_LASER_EXIST			= 1140,		//激光点已经存在
	TC_ANNOT_ERROR_CUSTOMERTYPE_NOTEXIST = 1141,	//自定义标注类型不存在
	...
	TC_AS_ERROR_P2P_CONNECTION_FAILED	= 1219,		//P2P连接出错
	TC_AS_ERROR_FIRST_USER_JOIN			= 1220,		//P2P连接出错
	...
	TC_VIDEO_ERROR_OVER_MAX_RESOLUTION_COUNT	= 1368,	//上层传的高低流不符合规格
	TC_VIDEO_ERROR_OVER_MAX_RESOLUTION			= 1369,	//上层观看的用户的真实分辨率超过设置的分辨率
	...
	TC_AUDIO_CREATE_SEND_THREAD_FAIL	= 1405,		//创建线程失败,导致mic声音无法发送到服务器
	TC_AUDIO_PARAM_WRONG				= 1406,		//参数错误
	TC_AUDIO_FUNC_ALREADY_CALLED		= 1407,		//多次调用
	TC_AUDIO_ENGINE_INIT_FAILED			= 1408,		//音频引擎初始化失败
	TC_AUDIO_PARAM_NOT_INITIALIZATION   = 1409,		//未调用tup_conf_audio_setParam设置参数
	TC_AUDIO_ERROR_SESSION_NOT_READY    = 1410,		//通道未加入
	TC_AUDIO_ERROR_MIC_NOT_OPEN			= 1411,		//麦克风未打开
	TC_AUDIO_ERROR_SPEAKER_NOT_OPEN     = 1412,		//扬声器未打开
	TC_AUDIO_MIC_ALREADY_OPEN			= 1413,		//麦克已经打开
	TC_AUDIO_MIC_ALREADY_MUTE           = 1414,		//麦克已经静音
	TC_AUDIO_MIC_ALREADY_UMUTE          = 1415,		//麦克已经打开
	TC_AUDIO_OVER_MAX_DEVICE_OPEN		= 1416,		//超过容许的打开设备上限
	TC_AUDIO_SPEAKER_ALREADY_OPEN       = 1417,		//扬声器已打开
	TC_AUDIO_SPEAKER_ALREADY_MUTE       = 1418,		//扬声器已静音
	TC_AUDIO_SPEAKER_ALREADY_UMUTE      = 1419,		//扬声器已经处于非静音
	TC_AUDIO_IS_NOT_SUPPORTED           = 1420,		//接口不被支持
	TC_AUDIO_ERROR_ALREADY_IN_PRIVATE   = 1421,		//已在私聊
	TC_AUDIO_ERROR_ALREADY_NOT_IN_PRIVATE = 1422,	//不在私聊中
	TC_AUDIO_ERROR_DEVICE_INOPERABLE    = 1423,		//多会议中，设备被其他会议占用
	TC_AUDIO_ERROR_CONF_SUPPEND         = 1424,		//会议已挂起

	TC_AUDIO_ERROR_PHONE_MGW_CODEC_FAIL			= 1425,		//网关编解码协商失败
	TC_AUDIO_ERROR_PHONE_MGW_VA_ALREADY_NOTIFY	= 1426,		//网关已通知
	TC_AUDIO_ERROR_PHONE_MGW_VA_ALREADY_TALK	= 1427,		//网关已通话
	...
	TC_AUDIO_ERROR_MGW_CODEC_ALREADY_SUCCESS	= 1432,		//网关编解码协商成功
...
enum
{
    ...
    CONF_MSG_ON_CONFERENCE_LEAVE        = 1003,	//会议离开通知
    CONF_MSG_ON_COM_STATUS_UPDATE       = 1004,	//通知组件切换
    CONF_MSG_ON_PRIVILEGE_UPDATE        = 1005,	//权限变更通知
    CONF_MSG_ON_CONF_MODEL_UPDATE       = 1006,	//会议模式变更通知
    CONF_MSG_ON_LAYOUT_UPDATE           = 1007,	//会议布局变更通知
	...
	CONF_MSG_ON_AUTH					= 1061, //请求验证，用于PaaS模式下私钥外部校验

	CONF_MSG_MAX						= 1099, //最大值，用于条件条件判断方便

    //////////////////////////////////////////////////////////////////////////
    // 会议用户消息
	CONF_MSG_USER_BASE = 1100,
	...
	CONF_MSG_USER_MAX						= 1199, //最大值，用于条件条件判断方便


	CONF_MSG_PHONE_BASE = 1200,
	...
	CONF_MSG_ON_PHONE_CALL_FAILED				 = 1222,	//呼叫电话失败通知
	CONF_MSG_ON_PHONE_CALL_SUCCESSED			 = 1223,	//呼叫电话成功通知

	CONF_MSG_ON_PHONE_EXTEND					 = 1224,	//延长的时间
	CONF_MSG_ON_PHONE_ROLLCALL_IND               = 1225,    //点名开启或禁用通知 nvalue1 == 1为开启，nvalue1 == 0为禁用

	CONF_MSG_PHONE_MAX							 = 1299,	//最大值，用于条件条件判断方便
    //////////////////////////////////////////////////////////////////////////
    // VIDEO MESSAGE
	COMPT_MSG_VIDEO_BASE = 2000,
...
//for  Android
	...
	COMPT_MSG_VIDEO_ON_SNAPSHOTDATA				= 2029,		//返回截图的内容

    COMPT_MSG_VIDEO_MAX							= 2099,		//最大值，用于条件条件判断方便
	...
	COMPT_MSG_AS_MAX						= 2199,		//最大值，用于条件条件判断方便

   //document share component
	COMPT_MSG_DS_BASE = 2200,
    ...
    COMPT_MSG_FT_ON_FILE_INFO			= 2302,	//文件项增加或状态更新
    COMPT_MSG_FT_ON_PROGRESS			= 2303,	//文件接收或发送进度
    COMPT_MSG_FT_ON_END					= 2304,	//发送或接收完成，或异常终止
    COMPT_MSG_FT_ON_READLY				= 2305,	//文件模块
	
	COMPT_MSG_FT_MAX					= 2399,	//最大值，用于条件条件判断方便

    //chat
	COMPT_MSG_CHAT_ON_GROUP_CREATE		= 2400,		//聊天群创建成功通知
	COMPT_MSG_CHAT_ON_GROUP_INFO_MODIFIED = 2401,	//聊天群信息修改通知
    COMPT_MSG_CHAT_ON_GROUP_INVITE		= 2402,		//收到邀请群聊的通知
    COMPT_MSG_CHAT_ON_GROUP_KICKOUT		= 2403,		//收到被踢出群聊的通知
	COMPT_MSG_CHAT_ON_GROUP_DESTROY		= 2404,		//创建者主动销毁组聊时通知

	COMPT_MSG_CHAT_ON_GROUP_USER_ONLINE	= 2405,		//组聊中用户上线
	COMPT_MSG_CHAT_ON_GROUP_USER_OFFLINE= 2406,		//组聊中用户下线
	COMPT_MSG_CHAT_ON_GROUP_USER_LEAVE	= 2407,		//组聊中用户退出

    COMPT_MSG_CHAT_ON_RECV_MSG			= 2408,		//收到聊天消息
	COMPT_MSG_CHAT_ON_PUBLIC_MSG_REMOVE	= 2409,		//删除公聊消息通知

	COMPT_MSG_CHAT_MAX					= 2499,		//最大值，用于条件条件判断方便

	//mediashare
	COMPT_MSG_MS_ON_STARTPLAY			= 2501,		//
	COMPT_MSG_MS_ON_PAUSEPLAY			= 2502,	
	COMPT_MSG_MS_ON_RESUMEPLAY			= 2503,	
	COMPT_MSG_MS_ON_STOPPLAY			= 2504,	
	COMPT_MSG_MS_ON_SEEKPLAY			= 2505,	
	COMPT_MSG_MS_ON_MIDPLAY				= 2506,	
	COMPT_MSG_MS_ON_PLAYERSTATE_CHANGE	= 2507,	
	COMPT_MSG_MS_ON_MEDIA_INFO			= 2508,	

	COMPT_MSG_MS_MAX					= 2599,		//最大值，用于条件条件判断方便

	//polling
	COMPT_MSG_POLLING_ON_COMP_READY		= 2601,				//投票组件准备好
	COMPT_MSG_POLLING_ON_RECV_POLL		= 2602,				//收到新的问卷
	COMPT_MSG_POLLING_ON_RECV_ANSWER	= 2603,				//收到新的答案
	COMPT_MSG_POLLING_ON_DELETE_POLL	= 2604,				//收到删除问卷指令
	COMPT_MSG_POLLING_ON_POLL_ID_CHANGED		= 2605,		//问卷ID发生变化
	COMPT_MSG_POLLING_ON_GLOBAL_PARAM_UPDATE	= 2606,		//全局参数发生变化
	COMPT_MSG_POLLING_ON_POLL_PARAM_UPDATE		= 2607,		//问卷参数更新消息

	//for android
	COMPT_MSG_POLLING_XML_ON_NEW_POLL				= 2610,			//创建新问卷，返回问卷ID
	COMPT_MSG_POLLING_XML_ON_NEW_QUESTION			= 2611,			//创建新问题，返回问题ID
	COMPT_MSG_POLLING_XML_ON_GET_POLL_TITLE			= 2612,			//获取问卷标题
	COMPT_MSG_POLLING_XML_ON_GET_POLL_USERDATA		= 2613,			//获取问卷用户数据
	COMPT_MSG_POLLING_XML_ON_GET_POLL_COUNT			= 2614,			//获取问卷数量
	COMPT_MSG_POLLING_XML_ON_GET_POLLID_BYINDEX		= 2615,			//根据索引获取问卷ID
	COMPT_MSG_POLLING_XML_ON_GET_CREATOR_USERID		= 2616,			//获取问卷创建者用户ID
	COMPT_MSG_POLLING_XML_ON_GET_QUESTION_COUNT		= 2617,			//获取某问卷问题数量
	COMPT_MSG_POLLING_XML_ON_GET_QUESTION			= 2618,			//获取某问卷问题类型、问题内容、问题选项
	COMPT_MSG_POLLING_XML_ON_GET_QUESTION_USERDATA	= 2619,			//获取问题用户数据
	COMPT_MSG_POLLING_XML_ON_GET_QUESTIONID_BYINDEX	= 2620,			//根据索引获取问题ID
	COMPT_MSG_POLLING_XML_ON_GET_QUESTION_ANSWER	= 2621,			//获取问题答案
	COMPT_MSG_POLLING_XML_ON_GET_QUESTION_ANSWER_USERDATA	= 2622,	//获取问题答案用户数据
	COMPT_MSG_POLLING_XML_ON_LOAD_FROM_FILE			= 2623,			//获取加载的问卷数量以及问卷ID

	COMPT_MSG_POLLING_MAX							= 2699,			//最大值，用于条件条件判断方便

	//audio
	COMPT_MSG_AUDIO_ON_AUDIO_MUTE_ALL_ATTENDEE		= 2701,		//收到会场静音消息

	COMPT_MSG_AUDIO_ON_ENGINE_DEVICE_CHANGE			= 2702,		//音频设备热插拔
	COMPT_MSG_AUDIO_ON_ENGINE_RUNNING_ERR_NOTIFY	= 2703,		//引擎系统运行时错误
	COMPT_MSG_AUDIO_ON_ENGINE_DEV_PLUGIN_NOTIFY		= 2704,		//有线耳机或者蓝牙耳机连接状态发生变化（android），该通知在引擎启动后生效
	COMPT_MSG_AUDIO_ON_ENGINE_ROUTE_CHANGE_NOTIFY	= 2705,		//移动路由状态发生变化（移动平台）

	COMPT_MSG_AUDIO_ON_AUDIO_NEGOTIATE_CODEC		= 2706,		//与服务器协商编解码结果
	COMPT_MSG_AUDIO_ON_AUDIO_ACTION					= 2707,	    //收到mic控制命令,以及进入私聊命令与离开私聊通知
	COMPT_MSG_AUDIO_ON_AUDIO_MAX_OPEN_DEVICE		= 2708,		//最大容许打开的设备数量
	COMPT_MSG_AUDIO_ON_AUDIO_DEVICE_STATUS_CHANGE	= 2709,		//音频设备状态通知 mic
	COMPT_MSG_AUDIO_ON_AUDIO_MGW_EXCEPTION			= 2710,     //MGW异常
	COMPT_MSG_AUDIO_ON_AUDIO_MGWVA_NEGOTIATE_CODEC	= 2711,		//语音助理
	COMPT_MSG_AUDIO_ON_AUDIO_OPEN_MIC				= 2712,		//异步打开mic结果

	COMPT_MSG_AUDIO_ON_SYSTEM_SPEAKER_VOLUME		= 2713,		//系统扬声器音量改变通知
	COMPT_MSG_AUDIO_ON_SYSTEM_MIC_VOLUME			= 2714,		//系统麦克风音量改变通知

	COMPT_MSG_AUDIO_ON_XML_MIC_VOLUME				= 2780,		//Mic音量变更通知
	COMPT_MSG_AUDIO_ON_XML_SPEAKER_VOLUME			= 2781,		//扬声器音量变更通知

	COMPT_MSG_AUDIO_MAX								= 2799,		//最大值，用于条件条件判断方便
	...
	COMPT_MSG_WB_MAX								= 2899,		//白板最大消息，没有实际意义
	...
	COMPT_MSG_ANNO_MAX				= 2999,			//ANNOTATION最大消息，没有实际意义
};
...
#define TC_MAX_RSA_KEY_LEN			1680 //RSA 2048位，私钥长度1675个
#define TC_MAX_ISV_ACCOUNT_LEN		37  //Account为UUID，36个字符
...
typedef struct tag_conf_user_record
{
	...
	TUP_CHAR					local_ip[TC_MAX_CONF_SERVER_IP];
} TC_Conf_User_Record;
...
typedef struct tag_init_paramter
{
	...
	TUP_UINT8			conf_mode;					//会议模式，1为PaaS模式，0为产品模式
} Init_param;

typedef struct tagIsvParam
{
	TUP_CHAR			isv_account[TC_MAX_ISV_ACCOUNT_LEN]; //ISV帐号，PaaS模式下需要填写
	TUP_CHAR			rsa_key[TC_MAX_RSA_KEY_LEN]; //RSA私钥
}Isv_Param;
//////////////////////////////////////////////////////////////////////////
//会议参数
typedef struct tag_conf_info
{
    ...
    TUP_CHAR                user_log_uri[TC_MAX_USER_LOG_URI_LEN];  //用户统一描述符，一般用于和其他系统集成时用户绑定，可选,PAAS模式下，为Token值

	CONF_OS_TYPE			os_type;			 //操作系统类型
	CONF_DEVICE_TYPE		dev_type;			 //设备类型
...
typedef struct tagTC_VIDEO_PARAM
{
	...
	TUP_UINT32	nRawtype;			//视频格式：如YUV，当前无效	//移动端可作为旋转角度传进来
...
//sharing Type
#define AS_SHARINGTYPE_DESKTOP			0x0000	//全屏共享
#define AS_SHARINGTYPE_APP				0x0001	//程序共享
//sharing session
...
typedef struct tagAnno_Resource_Item
{
	...
	TUP_UINT16	picWidth;			//图片宽度，TWIPS为单位，所有互通终端需要相同
	TUP_UINT16	picHeight;			//图片高度，TWIPS为单位，所有互通终端需要相同
	TC_POINT	ptOffset;			//图片中心点偏移量，TWIPS为单位，所有互通终端需要相同
} Anno_Resource_Item;
...
//定义页面标注的类型
#define DS_ANNOT_TYPE_LASERPOINTER	11 	// 激光点 Like laser pointer which moves as mouse moves
#define	DS_ANNOT_TYPE_DRAWING   	12 	// Hilight belongs to drawing
#define	DS_ANNOT_TYPE_CUSTOMER		13 	//自定义标注（图片类）
#define	DS_ANNOT_TYPE_TEXT			14	//文字标注


//定义页面标注的子类型
#define	DS_ANNOTDRAWING_TYPE_FREEHAND				1	//铅笔线
#define	DS_ANNOTDRAWING_TYPE_HILIGHT				2	//高亮线
#define	DS_ANNOTDRAWING_TYPE_RECTANGLE				3	//空心矩形
#define	DS_ANNOTDRAWING_TYPE_FILLRECTANGLE			4	//填充的矩形
#define	DS_ANNOTDRAWING_TYPE_ROUNDRECTANGLE			5	//空心圆角矩形
#define	DS_ANNOTDRAWING_TYPE_FILLROUNDRECTANGLE		6	//填充的圆角矩形
#define	DS_ANNOTDRAWING_TYPE_ELLIPSE				7	//空心椭圆
#define	DS_ANNOTDRAWING_TYPE_FILLELLIPSE			8	//填充的椭圆
#define	DS_ANNOTDRAWING_TYPE_LINE					9	//直线
#define	DS_ANNOTDRAWING_TYPE_LINEARROW				10	//单箭头直线
#define	DS_ANNOTDRAWING_TYPE_LINEDOUBLEARROW		11	//双箭头直线
#define	DS_ANNOTDRAWING_TYPE_WBPEN					12	//电子白板笔
#define	DS_ANNOTDRAWING_TYPE_WBERASE				13	//电子白板橡皮擦
#define DS_ANNOTDRAWING_TYPE_PAGEFRAME				14	//白板边框


typedef enum
{
	DS_HITTEST_CODE_ERROR = 0, 				// 未选中
	DS_HITTEST_CODE_INSIDE,					// 选中
	DS_HITTEST_CODE_RESIZE_UPPERLEFT,		// 左上角的拉伸点
	DS_HITTEST_CODE_RESIZE_UPPERMIDDLE,		// 上边中间的拉伸点
	DS_HITTEST_CODE_RESIZE_UPPERRIGHT,		// 右上角的拉伸点
	DS_HITTEST_CODE_RESIZE_MIDDLERIGHT,		// 右边中间的拉伸点
	DS_HITTEST_CODE_RESIZE_BOTTOMRIGHT,		// 右下角的拉伸点
	DS_HITTEST_CODE_RESIZE_BOTTOMMIDDLE,	// 下边中间的拉伸点	
	DS_HITTEST_CODE_RESIZE_BOTTOMLEFT,		// 左下角的拉伸点
	DS_HITTEST_CODE_RESIZE_MIDDLELEFT		// 左边中间的拉伸点
} DS_HITTEST_CODE;
...
typedef enum
{
	...
	DsRotate90FlipNone		= 1,	//顺时针旋转90度，未翻转
	DsRotate180FlipNone		= 2,	//顺时针旋转180度，未翻转
	DsRotate270FlipNone		= 3,	//顺时针旋转270度，未翻转
...
typedef struct 
{
	TUP_UINT16	zoomType;		//缩放类型
	TUP_UINT16	zoomPercent;	//缩放百分比
...
typedef struct tagCreateCustomerData
{
    ...
    TC_RECT2	dispRect;			//图片显示区域,当bLocal为1时，dispRect的宽高必须与初始化资源时传入的显示宽高相同
...
typedef struct tagDsDrawDataNotify	//界面更新时通知上层的数据结构
{
    TUP_VOID*	 pData;				//图像数据指针
    TUP_INT		nWidth;				//图像宽度（TWIPS)
    TUP_INT		nHeight;			//图像高度（TWIPS)
    TC_RECT2*	drawRect;			//更新区域，如果为NULL，则全图像更新
} DsDrawDataNotify;
...
{
	TUP_UINT16 extendType;		//子类型的ID，所有互通终端需要相同
	TUP_UINT16 flag;			//该子类型标注的属性，所有互通终端需要相同
}DsDefineAnnot;
