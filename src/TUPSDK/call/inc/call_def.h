...
#define TUP_CALL_VERSION   "TUP V100R001C50SPC300B011"
...
typedef enum tagCALL_E_DTMF_TONE
{
    ...
    CALL_E_DTMFFLASH,   /**< dtmf按键FLASH，预留，暂无使用 */
    CALL_E_BUTT
} CALL_E_DTMF_TONE;
...
typedef enum tagCALL_E_CALL_EVENT
{
    ...
    CALL_E_EVT_TO_UI_JOINT_UNHOLD,          /**< 联动恢复请求
                                                ...
                                                <br>param2：无
                                                <br>data：  CALL_S_JOINT_CMD_INFOS*   */    
    CALL_E_EVT_TO_UI_JOINT_BEHOLD,          /**< 联动被保持
                                                <br>param1：ulSipAccountID
                                                <br>param2：无
                                                <br>data：  CALL_S_JOINT_CMD_INFOS*   */    
    CALL_E_EVT_TO_UI_JOINT_CANCELBEHOLDED,  /**< 联动取消被保持
                                                <br>param1：ulSipAccountID
                                                <br>param2：无
                                                <br>data：  CALL_S_JOINT_CMD_INFOS*   */    
    ...
    CALL_E_EVT_TO_UI_JOINT_PEER_CAPS_REPORT,/**< 联动话机是否支持视频能力上报
                                                <br>param1：无
                                                <br>param2：无
                                                <br>data：  CALL_S_JOINMSCONF_INFO*  */    
    ...
    CALL_E_EVT_SERVERCONF_CREATE_RESULT,    /**< 创建会议结果
                                                <br>param1：ulConfID
                                                <br>param2：ulResult
                                                <br>data：  CALL_S_CONF_CREATE_RESULT */    
    ...
    CALL_E_EVT_RELEASE_LICENSE_RESULT,      /**< 释放license响应
                                                ...
                                                <br>param2：无
                                                <br>data：  无  */    
    CALL_E_EVT_IDO_OVER_BFCP,               /**< IDO会控使用bfcp方式
                                                <br>param1：ulCallID
    ...
    CALL_E_EVT_DIALOG_INFO,                 /**<接受到对话内INFO消息
                                                <br>param1：呼叫ID
                                                <br>param2：无
                                                <br>data：  CALL_S_DIALOG_INFO* */    
    CALL_E_EVT_CALL_INIT_FINISH,            /**<呼叫组件初始化完成通知
                                                <br>param1：0 成功 非0 不成功
                                                <br>param2：无
                                                <br>data：  无 */    
    CALL_E_EVT_TC_REBOOT_NOTIFY,           /**<TC长时间未重启的通知
                                                <br>param1：无
                                                <br>param2：无
                                                <br>data：  无 */ 
    CALL_E_EVT_CALL_HEAD_IMG_UPDATE,       /**< 自身头像更新通知 (在UC2.0+IMS组网下，话机使用此通知消息)
                                                <br>param1：ulSipAccount
                                                <br>param2：无
                                                <br>data：  头像id (以\0结尾的字符串)*/  
    CALL_E_EVT_SERVICE_CFG_UPDATE,         /**< 业务配置更新(由服务器侧触发)通知，业务界面需要保存更新配置，以及根据需要触发设备重启，使用配置生效
                                                <br>param1：ulSipAccount
                                                <br>param2：bNeedReboot   是否重启生效
                                                <br>data：  无 */  
    CALL_E_EVT_BUTT
} CALL_E_CALL_EVENT;
...
{
    CALL_E_PLAY_FILE_MODE_LOCAL = 0,      /**< 0表示播放本地文件 */
    CALL_E_PLAY_FILE_MODE_TC = 1,         /**< 1表示播放TC铃音 */
    CALL_E_PLAY_FILE_MODE_TC_LOCAL = 2,   /**< 2表示TC模式下播放本地文件 */
...
{
    CALL_E_PLAY_MEDIA_TYPE_TALK  = 0,     /**< 表示通话相关媒体 */
    CALL_E_PLAY_MEDIA_TYPE_MUSIC = 1,     /**< 表示普通本地媒体 */
...
typedef struct tagCALL_S_CALL_PARAMS
{
    ...
    TUP_INT8 acTelNum2[CALL_D_MAX_LENGTH_NUM];  /**< 电话号码, CTD被叫 */
...
typedef struct tagCALL_S_AUDIO_STREAM_INFO
{
    ...
    TUP_UINT32 ulSendBitRate;                           /**< 发送比特率(bps) */
    TUP_UINT32 ulRecvBitRate;                           /**< 接收比特率(bps) */
...
typedef struct tagCALL_S_VIDEO_STREAM_INFO
{
    ...
    TUP_UINT32 ulVideoSendBitRate;                      /**< 编码码率(bps) */
    TUP_UINT32 ulVideoRecvBitRate;                      /**< 解码码率(bps) */
...
typedef enum tagCALL_E_CFG_MAJOR_TYPE
{
    ...
    CALL_E_CFG_COMPATIBLE_PARAM,/**< 兼容类参数 */
...
#define CALL_D_CFG_SERVER_REGING_SERVER   CFG_ID_DEF(CALL_E_CFG_SERVER,CALL_E_CFG_DATA_TYPE_STRUCTURE,0x23,0) /**< 已注册的sip服务器地址(话机同时注册则返回已注册的优先级最高的服务器)，对应结构体为CALL_S_SERVER_CFG，不可设置，只用于获取*/
...
#define CALL_D_CFG_SIP_USE_MAA_CONFCTRL     CFG_ID_DEF(CALL_E_CFG_SIP, CALL_E_CFG_DATA_TYPE_BOOL,   0x29, 0) /**< 是否使用MAA会控，默认否 */
#define CALL_D_CFG_SIP_NTF_REMOTECTRL       CFG_ID_DEF(CALL_E_CFG_SIP, CALL_E_CFG_DATA_TYPE_BOOL,   0x30, 0) /**< 是否上报带主被叫控的re-invite */
#define CALL_D_CFG_SIP_NTF_180_INSTEAD      CFG_ID_DEF(CALL_E_CFG_SIP, CALL_E_CFG_DATA_TYPE_BOOL,   0x31, 0) /**< 是否使用180代替183上报 */
#define CALL_D_CFG_SIP_PAI_PRIORITY         CFG_ID_DEF(CALL_E_CFG_SIP, CALL_E_CFG_DATA_TYPE_INT,    0x32, 0) /**< sip账号信令信息 PAI or From优先 */


#define CALL_D_CFG_SIPGLOBAL_CONTENT_TYPE   CFG_ID_DEF(CALL_E_CFG_SIPGLOBAL, CALL_E_CFG_DATA_TYPE_STRUCTURE, 0x1, 0) /**< 配置对话内INFO消息，对应数据结构CALL_S_CONTENT_TYPE，默认为空*/
...
#define CALL_D_CFG_MEDIA_ENABLE_DATA        CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0xE,0)  /**< 全局媒体配置:辅流开关，默认否 */
#define CALL_D_CFG_MEDIA_FORCE_IDR_INFO     CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0xF,0)  /**< 全局媒体配置:是否强制打开info idr请求的收发，默认否 */
#define CALL_D_CFG_MEDIA_SDP_CT             CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_INT,       0x10,0) /**< 全局媒体配置:sdp CT值，取值范围TUP未作限制，产品需要根据实际组网要求配置，默认值0。*/
#define CALL_D_CFG_MEDIA_LOOSE_PORT_NEGO    CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0x11,0) /**< 全局媒体配置:是否支持非严格端口协商，默认false, 即严格端口协商（收到的RTP/RTCP数据包的源端口都要和SDP内的一致）。 */
#define CALL_D_CFG_MEDIA_WND_PARAM          CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_STRUCTURE, 0x12, 0)/**< 全局媒体配置:视频显示窗口属性（拉伸/镜像/旋转），对应结构体CALL_S_VIDEO_RENDER_INFO，默认{1,1,1} */
#define CALL_D_CFG_MEDIA_IFRAME_METHOD      CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0x13,0) /**< 全局媒体配置:I帧控制，默认否 */
#define CALL_D_CFG_MEDIA_FLUID_CONTROL      CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0x14,0) /**< 全局媒体配置:流控，默认否 */
#define CALL_D_CFG_MEDIA_DEL_PORT0_LINE     CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0x15,0) /**< 全局媒体配置:删除无效媒体行，默认否 */
#define CALL_D_CFG_VIDEOSESSION_PREVIEWTYPE CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_INT,       0x16, 0)/**< 全局媒体配置:视频预览类型，0：不带任何预览，设置为2则为视频预览，默认为0， */
...
#define CALL_D_CFG_MEDIA_LOOSE_IP_NEGO      CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_BOOL,      0x18,0) /**< 全局媒体配置:是否支持非严格IP协商，默认false, 即严格IP协商（收到的RTP/RTCP数据包的源IP都要和SDP内的一致）。 */
#define CALL_D_CFG_MEDIA_MOS_NTFTYPE        CFG_ID_DEF(CALL_E_CFG_MEDIA,  CALL_E_CFG_DATA_TYPE_INT,       0x19,0) /**< 全局媒体配置:媒体mos值上报类型，0 上报原始mos值 1 上报模式等级，默认上报mos等级 */
...
typedef struct tagCALL_S_DEVICEINFO
{
    ...
    TUP_UINT32 CameraOrient;                        /**< 摄像头角度, 仅移动平台摄像头设备有值 */
} CALL_S_DEVICEINFO;
...
{
    CALL_E_PLAY_FILE_MODE  enPlayMode;      /**< 播放文件模式 */
    CALL_E_PLAY_MEDIA_TYPE enMediaType;     /**< 媒体文件类型 */
    TUP_UINT32  ulLoops;                    /**< 循环次数（0表示循环播放） */
    TUP_CHAR*   pcPlayFile;                 /**< 播放铃声文件名播放铃音路径（目前支持wav格式），enPlayMode为CALL_E_PLAY_FILE_MODE_LOCAL和CALL_E_PLAY_FILE_MODE_TC_LOCAL时有效并必须填写 */
    TUP_UINT32  ulTcFileValue;              /**< 文件对应的值，enPlayMode为CALL_E_PLAY_FILE_MODE_TC时有效并必须填写 */
...
typedef enum tagCALL_E_BFCP_START_ERROR
{
    ...
    CALL_E_BFCP_START_ERROR_FORCE_REQUEST_FAILED = 6,   /**< 作为客户端，抢发辅流失败 */
