...
const int MAX_IMAGE_HEIGHT = 200;// 自定义图片高度
const int MAX_IMAGE_WIDTH = 250;// 自定义图片宽度
...
//************************************************************************
BOOL CTools::IsProcessExistsFromName(LPCTSTR name)
{
	PROCESSENTRY32 pe;
...
//************************************************************************
std::string CTools::MakeMixedIMContent(CString& imgPath,CString & _accesscode)
{
	std::string cont_;
	...
	if (0 != _accesscode.GetLength())
	{
		cont_ = cont_ + ";" +  CTools::UNICODE2UTF(_accesscode);
	}
	else
	{
...
std::string CTools::MakePicTxtMessage(PICUMInfo& m_UMinfo,FontInfo& m_font,MsgToSend& msgTxtPic)
{
	...
	////处理本地表情等信息/////
 	vector<PicInfo> m_picInfo;   ///将图片格式化/////
	CTools::IMPicFormat(msgTxtPic,m_picInfo);
	std::string _content = MakeMixedIMContent(m_UMinfo.m_strLocalPath,m_UMinfo.UMAccessCode);
...
std::string CTools::num2str(TUP_INT64 i)
{
	char str[EMOTION_CUSTOM_MAXCOUNT]= {0};
	sprintf_s(str, sizeof(str), "%I64d", i);
