...
#define WM_INPUT_STATUS			(WM_USER+1040)			// 接收方输入状态
#define WM_USER_KICKOUT			(WM_USER+1041)			// 用户被踢通知
// 王飞
#define WM_IM_DRAG_DROP_HANDLE         (WM_USER+1042) //拖拽事件响应
#define WM_IM_DRAG_HTML_HANDLE         (WM_USER+1043) //拖拽html到输入窗口
#define WM_IM_DELETE_IMAGE_DLG         (WM_USER+1044) //删除显示图片参数对话框指针
#define WM_MESSAGE_NEW_IM_DIALOG       (WM_USER+1045) // 通知主框架创建IM或者SMS
//test
#define WM_DISPATCH_MESSAGE_IM			(WM_USER+1050)			//客户端程序调用eSDK UC的“取消消息回调”接口tup_im_setdispatchmessage，使eSDK UC不再向客户端程序上报消息

#define WM_SHOW_DOWNLOADIMAGE			(WM_USER+1051)
#define WM_SEND_IMAGEINFO				(WM_USER+1052)
#define WM_RECV_OFFLINE_IM				(WM_USER+1053)// 离线消息
...
#define SAFE_DELETEARRY(pPoint) 	if(NULL != pPoint)\
{\
	delete[] pPoint;\
	pPoint = NULL;\
}
