...
// OleImage.cpp : Implementation of COleImage
#include "stdafx.h"
#include "OleImage.h"
...
//
DWORD COleImage::GetIOleObjectOffset(void)
{
...
DWORD COleImage::m_dwOleObjectOffset = COleImage::GetIOleObjectOffset();

// 根据IOleObject接口指针得到COleImage类的指针
COleImage * COleImage::FromOleObject(IOleObject *lpOleObject)
{
	// 其实就是简单地用lpOleObject减去前面计算出来的偏移量
	return (COleImage *) ((UINT_PTR) lpOleObject - m_dwOleObjectOffset);
}

//
// 初始化成员
//
HRESULT COleImage::FinalConstruct()
{
	m_pDCs = NULL;
	m_pBitmaps = NULL;
	m_ppOldBmps = NULL;
...
//
HRESULT COleImage::LoadFromFile(BSTR lpszPathName, CRichEditCtrl *pContainer, UINT nMaxWidth)
{
	// 先清除以前加载过的图像
	FinalRelease();
...
// 同样也会被调用
//
HRESULT COleImage::OnDraw(ATL_DRAWINFO& di)
{
	...
	if (m_nFrmCount > 0) 
	{
		BitBlt(di.hdcDraw, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,m_pDCs[m_pFrmIndexes[m_iCurTime]], 0, 0, SRCCOPY);
	}
	return S_OK;
}
