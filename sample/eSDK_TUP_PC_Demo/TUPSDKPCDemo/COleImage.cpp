...
//
DWORD CCOleImage::GetIOleObjectOffset(void)
{
...
DWORD CCOleImage::m_dwOleObjectOffset = CCOleImage::GetIOleObjectOffset();

// 根据IOleObject接口指针得到COleImage类的指针
CCOleImage * CCOleImage::FromOleObject(IOleObject *lpOleObject)
{
	// 其实就是简单地用lpOleObject减去前面计算出来的偏移量
	return (CCOleImage *) ((UINT_PTR) lpOleObject - m_dwOleObjectOffset);
}

//
// 初始化成员
//
HRESULT CCOleImage::FinalConstruct()
{
	m_pDCs = NULL;
	m_pBitmaps = NULL;
	m_ppOldBmps = NULL;
...
//
HRESULT CCOleImage::LoadFromFile(BSTR lpszPathName, CRichEditCtrl *pContainer, UINT nMaxWidth)
{
	// 先清除以前加载过的图像
	FinalRelease();
...
// 同样也会被调用
//
HRESULT CCOleImage::OnDraw(ATL_DRAWINFO& di)
{
