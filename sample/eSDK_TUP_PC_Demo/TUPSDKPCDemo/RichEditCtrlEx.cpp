		...
		if (nObjectCount > 0) {
			...
			for (int i = iFirst; i < nObjectCount; i++) {
				if (m_pRichEditOle->GetObject(i, &reo, REO_GETOBJ_POLEOBJ) == S_OK) {
					...
					// 是COleImage对象时才能更新
					if (InlineIsEqualGUID(reo.clsid, CLSID_COleImage)) {
						// 更换帧
						CCOleImage *pOleImage = CCOleImage::FromOleObject(reo.poleobj);
...
HRESULT CRichEditCtrlEx::InsertImage(LPCTSTR lpszPathName,int index)
{
	...
	do {
		...
		// 创建COleImage实例
		hr = CoCreateInstance(CLSID_COleImage, NULL, CLSCTX_INPROC, IID_ICOleImage, (LPVOID*) &spOleImage);
		if (hr != S_OK) {
			break;
		}
